#!/usr/bin/env python3
"""
Агент уведомлений v3.0 — ALT Linux + MATE
...
LOCK_FILE = "/tmp/notify-agent.lock"
VERSION   = "3.0"
LOG_FILE  = f"/tmp/notify-agent-{os.environ.get('USER', 'unknown')}.log"
...
def _install_hooks():
    ...
    # 3. Сигналы — SIGTERM, SIGINT, SIGHUP (не только штатный выход)
    def _sig_handler(signum, frame):
        sig_name = signal.Signals(signum).name
        log.warning(f"Получен сигнал {sig_name} (#{signum}) — агент завершается")
        # Записать стек в момент сигнала
    ...
    log.info(
        f"Окружение: DISPLAY={os.environ.get('DISPLAY','?')} | "
        f"XAUTHORITY={os.environ.get('XAUTHORITY','?')} | "
        f"DBUS={os.environ.get('DBUS_SESSION_BUS_ADDRESS','?')[:40]}"
    )
...
class NotifyPopup(QWidget):
    ...
    def __init__(self, title: str, message: str, level: str, timeout: int):
        ...
        # Диагностика + повторное позиционирование
        def _reposition():
            desk   = QDesktopWidget()
            screen = desk.availableGeometry(desk.primaryScreen())
        ...
        def _log_pos():
            pos    = self.pos()
            geom   = self.geometry()
            desk2  = QDesktopWidget()
            scr2   = desk2.availableGeometry(desk2.primaryScreen())
            log.info(
                f"Popup pos после show: x={pos.x()} y={pos.y()} "
                f"w={geom.width()} h={geom.height()} | "
                f"screen={scr2.width()}x{scr2.height()} "
                f"origin=({scr2.x()},{scr2.y()}) | "
                f"want_x={scr2.x() + scr2.width() - POPUP_WIDTH - POPUP_MARGIN} "
                f"want_y={scr2.y() + scr2.height() - (geom.height() or 120) - POPUP_MARGIN}"
            )
        QTimer.singleShot(200, _log_pos)
        for delay in [0, 500, 2000, 5000, 10000]:
            QTimer.singleShot(delay, _reposition)
        if timeout > 0:
